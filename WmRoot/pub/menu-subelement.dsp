          ...
          %invoke wm.server.csrfguard:replaceSpecialCharacters%
            <script>
              ...
              if (is_csrf_guard_enabled && needToInsertToken) {
                ...
                document.write('<a id="a%value encode(htmlattr) url%" target="%ifvar target%%value $host%%value target%%else%body%endif%" href="%value encode(javascript) url%"> ' + label + ' %ifvar target% ... %endif% </a>');
              }
            </script>
          %endinvoke%
          %endscope%
