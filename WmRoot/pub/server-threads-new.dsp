            ...
            <TR>
               <TD>
		        ...
		        if(is_csrf_guard_enabled && needToInsertToken) {
		     	document.write("<A class='imagelink' href='javascript:document.htmlform_single_server_thread_dump.submit();' onClick=\"return populateForm(document.htmlform_single_server_thread_dump, 'threadID', '%value threadid encode(url)%');\">%value name encode(html)%</A>");
		       } else {
			document.write('<A class="imagelink" href="single-server-thread-dump.dsp?threadID=%value threadid encode(url)%">%value name encode(html)%</A>');
		     }
           </script>
				</TD>
