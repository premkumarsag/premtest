...
<SCRIPT LANGUAGE="JavaScript">
  ...
  function onClick (action) {
    if (action == "reset") {
        ...
        if(is_csrf_guard_enabled && needToInsertToken) {
			createForm("htmlform_service_add_edit", "service-add.dsp", "POST", "BODY");
			setFormProperty("htmlform_service_add_edit", "operation", "edit");
            setFormProperty("htmlform_service_add_edit", _csrfTokenNm_, _csrfTokenVal_);
			document.htmlform_service_add_edit.submit();
        } else {
  ...
  function typeChange () {
    ...
    if (selectedType == "java") {
		if(is_csrf_guard_enabled && needToInsertToken) {
			createForm("htmlform_service_add_java", "service-add.dsp", "POST", "BODY");
			setFormProperty("htmlform_service_add_java", "type", "java");
			setFormProperty("htmlform_service_add_java", _csrfTokenNm_, _csrfTokenVal_);
			document.htmlform_service_add_java.submit();
		} else {
			document.location = "service-add.dsp?type=java";
		}
    } else {
		if(is_csrf_guard_enabled && needToInsertToken) {
			createForm("htmlform_service_add_csrf", "service-add.dsp", "POST", "BODY");
			setFormProperty("htmlform_service_add_csrf", _csrfTokenNm_, _csrfTokenVal_);
			document.htmlform_service_add_csrf.submit();
		} else {
			document.location = "service-add.dsp";
		}
	}
