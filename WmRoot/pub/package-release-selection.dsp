                ...
                %loop released%
                    ...
                    <TR>
                        ...
                        <script>writeTDrowspan("rowdata",2);</script>
							<script>
							createForm("htmlform_package_release_selection_%value $index%", "package-release-selection.dsp", "POST", "BODY");
							setFormProperty("htmlform_package_release_selection_%value $index%", "action", "unrelease");
							setFormProperty("htmlform_package_release_selection_%value $index%", "releasedPackages", "%value name%");
							</script>
							<script>
								if(is_csrf_guard_enabled && needToInsertToken) {
									document.write('<A class ="imagelink" href="javascript:document.htmlform_package_release_selection_%value $index%.submit();"><IMG  SRC="icons/delete.png" border=0 alt="Delete %value name encode(htmlattr)%"></A>');
								} else {
